...
import org.openmrs.module.kenyadreams.reporting.library.patientsVisits.PatientVisitsIndicatorLibrary;
...
public class PatientClinicalVisitsReportBuilder extends AbstractReportBuilder {
	...
	protected List<Mapped<DataSetDefinition>> buildDataSets(ReportDescriptor reportDescriptor,
		...
		return Arrays.asList(ReportUtils.map(patientsEnrolledInHiv(), "")
	...
	protected DataSetDefinition patientsEnrolledInHiv() {
		...
		List<ColumnParameters> diffCareDisaggregations = Arrays.asList(all0_to14, f15Plus, m15Plus, colTot);
		
		cohortDsd.setName("1");
		cohortDsd.setDescription("Dataset for patients who are enrolled in HIV program");
		
		EmrReportingUtils.addRow(cohortDsd, "Patient visits", "",
		    ReportUtils.map(patientVisitsIndicatorLibrary.patientsEnrolledInHiv()), diffCareDisaggregations,
